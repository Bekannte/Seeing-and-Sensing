    ...
    for (let i = 0; i < this.notePos.length; i++) {
      for (let j = 0; j < this.notePos[i].length; j++) {
        ...
        if (this.noteState[i][j] > 0) {
          let alpha = this.noteState[i][j] * 100;
          let c1 = color(255, 0, 0, alpha);
          let c2 = color(0, 0, 255, alpha);
          let mix = lerpColor(c1, c2, map(i, 0, this.notePos.length, 0, 1));
          fill(mix);
          let s = this.noteState[i][j];
          rect(x, y, this.noteSize * s, this.noteSize * s);
          let c3 = color(255, 100, 0, alpha);
          let c4 = color(0, 100, 255, alpha);
          let mix1 = lerpColor(c3, c4, map(i, 0, this.notePos.length, 0, 1));
          fill(mix1);
          circle(x, y, this.noteSize * s+20, this.noteSize * s+20);
        }
